...
function FractalPlus2(x, y, width, height, maxDepth, currentDepth = 0) {
    ...
    for (let d = 0; d < 2 ** (currentDepth - 1); d++) {
        ...
        color += ((width + (d * 6 * width) + (offset * width)) / range[1] < x && x < (2 * width + (d * 6 * width) + (offset * width)) / range[1]) ? 254 / (currentDepth + 1) : 0;
        color += ((height + (d * 6 * height) + (offset * height)) / range[1] < y && y < (2 * height + (d * 6 * height) + (offset * height)) / range[1]) ? 254 / (currentDepth + 1) : 0;
    }
